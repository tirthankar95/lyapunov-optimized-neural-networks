...
class NN_relu:
    def __init__(self, lr = 0.001) -> None:
        ...
        while True:
            ...
            with torch.no_grad():
                Yp = self.relu(self.relu(self.X @ self.layer0 + self.bias0) @ self.layer1 + self.bias1)
    ...
    def predict(self):
        with torch.no_grad():
            return self.relu(self.relu((self.X @ self.layer0 + self.bias0)) @ self.layer1 + self.bias1)
    ...
    def __str__(self) -> str:
        print(self.layer0, self.bias0, \
              self.layer1, self.bias1)
        
def run_model():
    obj_arr = [NN_relu(g.learning_rate) for _ in range(g.n_children)]
    ...
    with open('RAW_DATA/ReLU.json', 'w') as file:
        json.dump({
                    "avg_dist": dist_arr,
                    "loss": loss_arr
                 }, file)
