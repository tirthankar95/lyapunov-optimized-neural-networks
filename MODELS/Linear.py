...
class NN_LINEAR:
    def __init__(self, lr = 0.001) -> None:
        ...
        while True:
            ...
            with torch.no_grad():
                Yp = (self.X @ self.layer0 + self.bias0) @ self.layer1 + self.bias1
    ...
    def once(self):
        ...
        return torch.mean((self.Y-Yp)**2).item()
    
    def predict(self):
        with torch.no_grad():
            return (self.X @ self.layer0 + self.bias0) @ self.layer1 + self.bias1
...
def run_model():
    obj_arr = [NN_LINEAR(g.learning_rate) for _ in range(g.n_children)]
    ...
    for i in range(1, len(obj_arr)):
        assert obj_arr[0].get_dist(obj_arr[i]) != 0, f'Wts should not be equal.'
    # UNIT TEST 3
    for i in range(1, len(obj_arr)):
        assert obj_arr[i].layer0.requires_grad == True, f'Gradient descent should be activated.'
    ...
    with open('RAW_DATA/Linear.json', 'w') as file:
        json.dump({
                    "avg_dist": dist_arr,
                    "loss": loss_arr
                 }, file)
